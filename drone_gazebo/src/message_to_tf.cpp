/*
* SOURCE : tu-darmstadt-ros-pkg/hector_localization
* https://github.com/tu-darmstadt-ros-pkg/hector_localization
* Several changes have been made to the source code, to adapt to our problem
* More specifically, I added the transform between g_child_frame_id and g_footprint_frame_id to take into account the
* translation in z and the roll, pitch for the orientation
*/
...
// IN MY CASE :
// odometry_topic is either odom OR ground_truth/state (for testing)
// frame_id is world
// footprint_frame_id is base_footprint
// position_frame_id is empty
// stabilized_frame_id is base_stabilized
...
void sendTransform(geometry_msgs::Pose const& pose, const std_msgs::Header& header, std::string child_frame_id = "")
{
  ...
  // Extended to use all of 6 DOF. Added z in position and roll, pitch
...
void multiCallback(topic_tools::ShapeShifter const& input)
{
  ...
  ROS_ERROR_THROTTLE(1.0, "message_to_tf received a %s message. Supported message types: nav_msgs/Odometry "
                          "geometry_msgs/PoseStamped geometry_msgs/TransformStamped sensor_msgs/Imu",
                     input.getDataType().c_str());
}
